...
std::string formatOutput(const std::string& output) {
    ...
    for (char ch : output) {
        if (newLine) {
            formattedOutput += std::string(indentLevel * 4, ' '); // Indentation
            newLine = false;
        }
        ...
        if (ch == '{') {
            ...
            formattedOutput += std::string(indentLevel * 4, ' ');
            formattedOutput += "}\n";
            newLine = true;
        } else if (ch == ';') {
            formattedOutput += ";\n";
...
int main(int argc, char *argv[]) {
    ...
    if (rootNode) {
        ...
        rootNode->postOrderTraversalToString(linearOutput);

        std::string formattedOutput = formatOutput(linearOutput);

        std::ofstream outlog("22241038_log.txt", std::ios::trunc);
        outlog << formattedOutput;
        outlog.close();
    }
